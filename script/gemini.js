...
module.exports .run = async function ({ api, event, args }) {
  ...
  try {
    const geminiApi = `https://gemini-23xn.onrender.com/api/gemini`;
    if (event.type == "message_reply") {
      if (event.messageReply.attachments[0]?.type == "photo") {
        url = encodeURIComponent(event.messageReply.attachments[0].url);
        const res = (await axios.get(`${geminiApi}?message=${encodeURIComponent(prompt)}`)).data;
        return api.sendMessage(res.data.message, event.threadID);
      } else {
        return api.sendMessage('Please reply to an image.', event.threadID);
      }
    }
    const response = (await axios.get(`${geminiApi}?message=${encodeURIComponent(prompt)}`)).data;
